...
def deploy(pbi_root, workspace, dataset_params=None, credentials=None, force_refresh="REPOSITORY", on_report_success=None, cherry_picks=None, config_workspace=None, release=None):
    error = False
    ...
    for dir in dirs:
        try: # Allow other report groups to deploy, even if others fail
            ...
            if force_refresh == "REPOSITORY":
                local_force_refresh = check_file_modified(dataset_file)
            elif force_refresh == "REPLACE":
                local_force_refresh = True
            elif force_refresh == "REUSE":
                local_force_refresh = False
            else:
                print(f'!! ERROR. Invalid value for the force_refresh parameter: {force_refresh}')
                error = True
                break
